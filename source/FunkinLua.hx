...
class FunkinLua {
	...
	public function new(script:String) {
		...
		Lua_helper.add_callback(lua, "mouseOverlap", function(obj1:String, camera:String) {
			var namesArray:Array<String> = [obj1];
			var objectsArray:Array<FlxSprite> = [];
			for (i in 0...namesArray.length)
			{
				var real = PlayState.instance.getLuaObject(namesArray[i]);
			...
			if(!objectsArray.contains(null) && CoolUtil.mouseOverlaps(objectsArray[0],cameraFromString(camera)))
			{
				return true;
			}
			return false;
		});
		Lua_helper.add_callback(lua, "mouseOverlaps", function(obj1:String, camera:String) {
			var namesArray:Array<String> = [obj1];
			var objectsArray:Array<FlxSprite> = [];
			for (i in 0...namesArray.length)
			{
			...
			if(!objectsArray.contains(null) && FlxG.mouse.overlaps(objectsArray[0],cameraFromString(camera)))
			{
				return true;
			}
			return false;
		...
		Lua_helper.add_callback(lua, "startVideoSprite", function(videoFile:String) {
			#if VIDEOS_ALLOWED
	...
	function loadFrames(spr:FlxSprite, image:String, spriteType:String)
	{
		switch(spriteType.toLowerCase().trim())
		{
			...
			default:
				if (image.contains('character')) {
					//if (ClientPrefs.superfuckingHD) image = 'HD/' + image;
				}
